...
async function showSelectedRevendasTerritories() {
    ...
    for (const revenda of selectedRevendas) {
        try {
            ...
            if (data.success && data.data && Object.keys(data.data).length > 0) {
                ...
                // Create a layer object for the revenda
                const revendaLayer = {
                    id: `revenda-${revenda.id}`,
                    name: `Território - ${revenda.nome}`,
                    type: 'revendas',
                    color: revenda.cor || '#FF5722',
                    revendaId: revenda.id,
                    visible: true,
                    data: data.data // Store the municipality data
                };

                // Load the territory on the map
                await loadMunicipalityBoundariesForRevendasLayer(revendaLayer, data.data, { min: 1, max: 1 });

                if (revendaLayer.mapLayer && revendaLayer.mapLayer.getBounds().isValid()) {
                    allBounds.extend(revendaLayer.mapLayer.getBounds());
                    hasValidBounds = true;
                }
            } else {
                console.error('Erro ao carregar território da revenda:', data.error || 'Nenhum município encontrado');
                console.log('Data received:', data);
                alert(`Erro ao carregar território da revenda ${revenda.nome}: ${data.error || 'Nenhum município encontrado no território'}`);
            }
        } catch (error) {
    ...
    if (hasValidBounds) {
        ...
        console.warn('Nenhum território válido foi carregado');
        alert('Nenhum território válido foi encontrado para as revendas selecionadas');
    }
